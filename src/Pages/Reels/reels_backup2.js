...
const Item = ({ level, id }) => {
  ...
  useEffect(() => {
    const getuserdetails = async () => {
      try {
        const details = await axios.get(`http://localhost:5000/api/user/post/user/details/${id.user}`);
        console.log(details);
  ...
  const handleReaction5 = async () => {
       ...
       await fetch(`http://localhost:5000/api/reels/${id._id}/react_5_reel`,
         {
           method: "PUT",
           headers: {
...
`;
  ...
  const addCommentToReel=async()=>{
    await fetch(`http://localhost:5000/api/reels/${id._id}/commentreel`,
    {
      method: "PUT",
      headers: {
        'Content-Type': 'application/json',
  ...
  return (
    <div className={className} style={{ display: "flex", flexDirection: "column" }}>
      ...
      {
        className==="item level0"&& <div  class="All_comments">

          {
            id.comments && id.comments.map((comm)=>{
              return <div class = 'comment_container' style={{ marginTop:'-160px',marginLeft:'456px',display:'flex',flexDirection:'row',width:'333px',borderRadius:'20px 30px 30px 0px',border:'1px solid white', backgroundColor: 'rgba(255, 255, 255, 0.2)',backdropFilter: 'blur(10px)',boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)'}}>
              <img
                  src={comm.profilepic}
                alt=""
                className="PostImage"
                ...
                />
                <p style={{ display:'flex' , alignItems:'center' , justifyContent:'center', fontSize:'13px' , lineHeight:'15px' , textAlign:'start' , padding:'7px'}}>{comm.comment}</p>
              </div>
            })
          }
...
<div  class="btn position-relative" onClick={startAnimation_1} style={{ cursor: 'pointer' }}>
<svg 
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 48 48"
      width="35"
    ...
    <div  class="btn position-relative" onClick={startAnimation_2} style={{ cursor: 'pointer' }}>
<svg
      id="_01-smile"
      data-name="01-smile"
      xmlns="http://www.w3.org/2000/svg"
    ...
    <div  class="btn position-relative" onClick={startAnimation_3} style={{ cursor: 'pointer' }}>
    ...
    <div  class="btn position-relative" onClick={startAnimation_4} style={{ cursor: 'pointer' }}>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="35" height="35">
      <defs>
    ...
    <div  class="btn position-relative"  onClick={() => { startAnimation_5();handleReaction5(); console.log(video_Id); }} style={{ cursor: 'pointer' }}>
<Emoji/>
<span class="position-absolute top-0 start-10 translate-middle badge rounded-pill bg-danger">
        {R5count-1}
  </span>
